...
class Processer():
    ...
    def convert_mm_examples_to_features(self, examples, label_list, tokenizer, data_name):
        ...
        transform = transforms.Compose([
            ...
            transforms.Normalize((0.485, 0.456, 0.406),
                                 (0.229, 0.224, 0.225))])
        if not os.path.exists(data_name+"_images.pkl"):
            ...
            f = open(data_name+"_images.pkl", "wb")
            pickle.dump(features, f)
        else:
            f = open(data_name+"_images.pkl", "rb")
    ...
    def emotion_process(self, input_idx, wordid2emotionid):
        total_emotion_input_ids = []
        input_idx_list = input_idx.numpy()
        total_sample_shape = input_idx_list.shape
        print("Input ids shape is: ", total_sample_shape)
        for i in range(total_sample_shape[0]):
            emotion_input_ids = []
            for j in range(total_sample_shape[1]):
                emotion_input_ids.append(wordid2emotionid[str(input_idx_list[i][j])])
            total_emotion_input_ids.append(emotion_input_ids)
        
        return torch.tensor(total_emotion_input_ids, dtype=torch.long)