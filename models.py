...
import pickle
BERT_VERSION = 'BERT'  # 'RoBERTa', 'BERT'
...
class MsdBERT(nn.Module):
    ...
    def forward(self, input_ids, visual_embeds_att, input_mask, added_attention_mask, hashtag_input_ids,
                hashtag_input_mask, imgs_f, input_emotion_ids, hashtag_input_emotion_ids, labels=None):
        # sequence_output, pooled_output = self.bert(input_ids=input_ids, token_type_ids=None, attention_mask=input_mask)
        res = self.bert(input_ids=input_ids, token_type_ids=None, attention_mask=input_mask)
        sequence_output = res['last_hidden_state']
        pooled_output = res['pooler_output']

        res_hash = self.hashtag_bert(input_ids=hashtag_input_ids, token_type_ids=None,
                                     attention_mask=hashtag_input_mask)
        hashtag_output = res_hash['last_hidden_state']
        hashtag_pooled_output = res_hash['pooler_output']
        ...
        # text_mask # b*75
        text_mask = added_attention_mask[:, :75]
        # img_mask # b*1*1*49
        ...
        text_img_cross_attn = self.text2image_attention(visual, sequence_output, extended_text_mask)
        # b*75*12
        ...
        text_image_pooled_output = self.image_text_pooler(text_img_cross_attn)

        pooled_output = torch.cat([image_text_pooled_output, text_image_pooled_output, hashtag_text_cross_attn.squeeze(1)], dim=-1)
...
# 引入外部情感词典的方法
class MsdEmoBERT(nn.Module):
    def __init__(self):
        super(MsdEmoBERT, self).__init__()
        if BERT_VERSION == 'RoBERTa':
            self.bert = RobertaModel.from_pretrained('roberta-base/')
            self.hashtag_bert = RobertaModel.from_pretrained('roberta-base/')
        else:
        ...
        self.dropout = nn.Dropout(0.1)
        self.vismap2text = nn.Linear(2048, 768)
        self.classifier = nn.ModuleList([nn.Linear(768 * 3 + self.emo_dim * 2, 2) for i in range(self.emo_head)])
        self.W_b = nn.Parameter(nn.init.xavier_uniform_(torch.FloatTensor(768, 768)))

        # emotion model
        self.vismap2visemo_1 = nn.Linear(2048, 1024)
        self.vismap2visemo_2 = nn.Linear(1024, self.emo_dim)
        self.mlp_dropout = nn.Dropout(0.1)
        self.relu = nn.ReLU()
        self.batch_norm = nn.BatchNorm1d(1024, momentum=0.5)
        # emotion tag embedding
        self.emo_embedding = nn.Embedding(10, self.emo_dim)
        self.emo_W_b = nn.Parameter(nn.init.xavier_uniform_(torch.FloatTensor(self.emo_dim, self.emo_dim)))
    ...
    def forward(self, input_ids, visual_embeds_att, input_mask, added_attention_mask, hashtag_input_ids,
        ...
        # text emotion sequence
        # b*75*768
        text_emo_input = self.emo_embedding(input_emotion_ids)
       # print("text_emo_input.shape:",text_emo_input.shape)

       # print("text_emo_input_squeeze.shape:", text_emo_input.shape)
        # b*10*768
        hashtag_emo_input = self.emo_embedding(hashtag_input_emotion_ids)

        # visual MLP b*768
        visual_emo = self.vismap2visemo_1(imgs_f)
        visual_emo = self.mlp_dropout(visual_emo)
        visual_emo = self.relu(visual_emo)
        visual_emo = self.batch_norm(visual_emo)
        visual_emo = self.vismap2visemo_2(visual_emo)

        visual_emo = visual_emo.view(-1, 1, self.emo_dim)
        # b*1*75
        # text作为query， image作为key和value
        vis_text_attn = torch.matmul(visual_emo, sequence_output.transpose(1, 2))
        # b*1*75
        vis_text_attn = F.softmax(vis_text_attn, dim=-1)
        # b*1*768
        vis_text_emo_attn = torch.matmul(vis_text_attn, text_emo_input).squeeze()
        with open('attention.pkl', 'wb') as f:
            pickle.dump(vis_text_emo_attn, f)
        #vis_text_emo_attn_numpy=vis_text_emo_attn.detach().cpu().numpy()
        #numpy.savetxt("result.txt", vis_text_emo_attn_numpy)

        E = self.tanh(torch.matmul(torch.matmul(text_emo_input, self.emo_W_b), hashtag_emo_input.transpose(1, 2)))
        # C: b*12
        E, _ = torch.max(E, dim=1)
        emo_attn = F.softmax(E, dim=-1)
        # b*1*768
        hashtag_text_cross_emo_attn = torch.matmul(emo_attn.unsqueeze(1), hashtag_emo_input)
        pooled_output = torch.cat([image_text_pooled_output.view(-1, 768),
                                   text_image_pooled_output.view(-1, 768),
                                   hashtag_text_cross_attn.view(-1, 768),
                                   self.lam * vis_text_emo_attn.view(-1, 768),
                                   #self.lam * text_emo_input.view(-1, 768),
                                   self.lam * hashtag_text_cross_emo_attn.view(-1, 768)], dim=-1)
        pooled_output = self.dropout(pooled_output)
        classifier_outout = []
        for k in range(1, self.emo_head):
            logits = self.classifier[k](pooled_output)
            classifier_outout.append(logits)
        logits = sum(classifier_outout) / self.emo_head
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits.view(-1, 2), labels.view(-1))
            return loss
